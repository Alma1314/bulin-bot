...
- Local runtime exposes `novabot_read_file_tool`, `novabot_file_write_tool`,
  `novabot_file_edit_tool`, and `novabot_grep_tool`.
- Sandbox runtime exposes `novabot_upload_file`, `novabot_download_file`,
  `novabot_read_file_tool`, `novabot_file_write_tool`,
  `novabot_file_edit_tool`, and `novabot_grep_tool`.
...
- Member + local: read/grep are restricted to `data/skills`,
  ...
  `data/workspaces/{normalized_umo}`, and `/tmp/.novabot`; write/edit are
...
def get_novabot_workspaces_path() -> str:
    ...
    return computer_util.get_novabot_workspaces_path()


def _workspace_root(umo: str) -> Path:
    """Workspace root that follows both util-level and fs-level getter monkeypatches."""
    normalized_umo = normalize_umo_for_workspace(umo)
    return (Path(get_novabot_workspaces_path()) / normalized_umo).resolve(strict=False)


def _plugin_skill_roots() -> tuple[Path, ...]:
    plugins_root = Path(get_novabot_plugin_path())
...
def _read_allowed_roots(umo: str) -> tuple[Path, ...]:
    ...
    return (
        Path(get_novabot_skills_path()).resolve(strict=False),
        *_plugin_skill_roots(),
        _workspace_root(umo),
        Path(get_novabot_system_tmp_path()).resolve(strict=False),
        Path(get_novabot_temp_path()).resolve(strict=False),
    )
...
def _is_restricted_env(context: ContextWrapper[NovaAgentContext]) -> bool:
...
class FileReadTool(FunctionTool):
    name: str = "novabot_file_read_tool"
...
class FileWriteTool(FunctionTool):
    name: str = "novabot_file_write_tool"
...
class FileEditTool(FunctionTool):
    name: str = "novabot_file_edit_tool"
...
class FileDownloadTool(FunctionTool):
    name: str = "novabot_download_file"
    description: str = (
