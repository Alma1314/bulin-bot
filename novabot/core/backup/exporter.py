...
class NovaBotExporter:
    ...
    async def export_all(
        ...
        if output_dir is None:
            output_dir = get_novabot_backups_path()

        # 确保输出目录存在
        Path(output_dir).mkdir(parents=True, exist_ok=True)

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        zip_filename = f"novabot_backup_{timestamp}.zip"
    ...
    async def _export_kb_documents(self, kb_helper: Any) -> dict[str, Any]:
        ...
        try:
            ...
            offset = 0
            while True:
                page = await vec_db.document_storage.get_documents(
                    metadata_filters={},
                    offset=offset,
                    limit=page_size,
                )
    ...
    def _generate_manifest(
        ...
        manifest = {
            ...
            "has_knowledge_bases": bool(self.kb_manager and self.kb_manager.kb_insts),
            "schema_version": {
                "main_db": "v4",
                "kb_db": "v1",
            },
