...
from novabot.core.skills.skill_manager import SANDBOX_SKILLS_ROOT, SkillManager
...
_MANAGED_SKILLS_FILE = ".novabot_managed_skills.json"
...
def _collect_sync_skill_dirs() -> list[tuple[str, Path]]:
    ...
    skills_root = Path(get_novabot_skills_path())
...
def _discover_bay_credentials(endpoint: str) -> str:
    ...
    # 2. Mono-repo layout: NovaBot/../pkgs/bay/credentials.json
    novabot_root = Path(__file__).resolve().parents[3]  # novabot/core/computer/ → root
    candidates.append(novabot_root.parent / "pkgs" / "bay" / "credentials.json")
