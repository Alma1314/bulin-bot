...
from novabot.core.db.po import Persona, PersonaFolder, Personality
from novabot.core.platform.message_session import MessageSession
from novabot.core.sentinels import NOT_GIVEN

_PERSONA_NONE_SENTINEL = "[%None]"
...
class PersonaManager:
    def __init__(self, db_helper: BaseDatabase, acm: NovaBotConfigManager) -> None:
        self.db = db_helper
        self.acm = acm
        default_conf = acm.default_conf
        default_ps = default_conf.get("provider_settings", {}) if default_conf else {}
    ...
    async def resolve_selected_persona(
        ...
        *,
        ...
        if not persona_id:
            ...
            if persona_id == _PERSONA_NONE_SENTINEL:
                pass
        ...
        if not persona and platform_name == "webchat" and persona_id != _PERSONA_NONE_SENTINEL:
    ...
    def get_v3_persona_data(
        ...
        """获取 NovaBot <4.0.0 版本的 persona 数据。
