...
from novabot.core.utils.io import ensure_dir, on_error
...
class RepoZipUpdator:
    ...
    async def fetch_release_info(self, url: str, latest: bool = True) -> list:
        ...
        try:
            ...
            raise Exception(f"解析版本信息失败: HTTP 状态码 {e.response.status_code}") from e
        except Exception as e:
            logger.error(f"解析版本信息时发生异常: {e}")
            raise Exception(f"解析版本信息失败: {e}") from e
        return ret
