...
from novabot.core import (
    DependencyConflictError,
    logger,
    pip_installer,
    sp,
)
from novabot.core.agent.handoff import FunctionTool, HandoffTool
from novabot.core.config.novabot_config import NovaBotConfig
from novabot.core.config.default import VERSION
from novabot.core.platform.register import unregister_platform_adapters_by_module
...
from novabot.core.utils.io import remove_dir
...
try:
    ...
    if os.getenv("NOVABOT_RELOAD", "0") == "1":
        logger.warning("未安装 watchfiles，无法实现插件的热重载。")


class PluginVersionIncompatibleError(Exception):
    """Raised when plugin novabot_version is incompatible with current NovaBot."""
...
class PluginManager:
    def __init__(self, context: Context, config: NovaBotConfig) -> None:
        ...
        self.config = config
        self.plugin_store_path = get_novabot_plugin_path()
        """存储插件的路径。即 data/plugins"""
        self.plugin_config_path = get_novabot_config_path()
        """存储插件配置的路径。data/config"""
        self.reserved_plugin_path = os.path.join(
            get_novabot_path(), "novabot", "builtin_stars"
        )
        """保留插件的路径。在 novabot/builtin_stars 目录下"""
    ...
    def _load_plugin_metadata(plugin_path: str, plugin_obj=None) -> StarMetadata | None:
        ...
        Notes: 旧版本 NovaBot 插件可能使用的是 info() 函数来获取元数据。
        """
        metadata = None
        ...
        if isinstance(metadata, dict):
            ...
            metadata = StarMetadata(
                ...
                novabot_version=(
                    metadata["novabot_version"]
                    if isinstance(metadata.get("novabot_version"), str)
                    else None
                ),
    ...
    def _load_plugin_i18n(plugin_path: str) -> dict[str, dict]:
        ...
        i18n_dir = plugin_root / ".novabot-plugin" / "i18n"
    ...
    @staticmethod
    def _validate_novabot_version_specifier(
        ...
        try:
            ...
            return (
                False,
                "Invalid novabot_version. Use a PEP 440 range, e.g. >=4.16,<5.",
            )
        ...
        try:
            ...
            return (
                False,
                f"Invalid current NovaBot version: {VERSION}. Cannot check plugin version range.",
            )

        if not specifier.contains(current_version, prereleases=True):
            return (
                False,
                f"NovaBot {VERSION} does not satisfy plugin novabot_version: {normalized_spec}",
            )
        return True, None
    ...
    def _get_plugin_related_modules(
        ...
        """
        prefix = "novabot.builtin_stars." if is_reserved else "data.plugins."
    ...
    def _purge_modules(
        ...
        Args:
            module_patterns: 要移除的模块名模式列表（例如 ["data.plugins", "novabot.builtin_stars"]）
    ...
    def _cleanup_plugin_state(self, dir_name: str, is_reserved: bool = False) -> None:
        plugin_root_name = "novabot.builtin_stars." if is_reserved else "data.plugins."
                ...
                record.update(
                    {
                        ...
                        "novabot_version": metadata.novabot_version,
                    }
                )
    ...
    def _rebuild_failed_plugin_info(self) -> None:
        ...
        for dir_name, info in self.failed_plugin_dict.items():
            if isinstance(info, dict):
                ...
                version = info.get("version") or info.get("novabot_version")
                if version:
                    lines.append(
        ...
        for plugin_module in plugin_modules:
            try:
                ...
                reserved = plugin_module.get(
                    "reserved",
                    False,
                )  # 是否是保留插件。目前在 novabot/builtin_stars 目录下的都是保留插件。保留插件不可以卸载。
                plugin_dir_path = (
                ...
                path = "data.plugins." if not reserved else "novabot.builtin_stars."
                ...
                if path in star_map:
                    ...
                    try:
                        ...
                        if metadata_yaml:
                            ...
                            metadata.novabot_version = metadata_yaml.novabot_version
                    ...
                    if not ignore_version_check:
                        is_valid, error_message = (
                            self._validate_novabot_version_specifier(
                                metadata.novabot_version,
                            )
                        )
                        if not is_valid:
                            raise PluginVersionIncompatibleError(
                                error_message
                                or "The plugin is not compatible with the current NovaBot version."
                            )
                    ...
                    if not classes:
                        raise Exception(
                            ...
                            "请确认插件主类继承 novabot.api.star.Star，或类名以 Plugin 结尾 / 命名为 Main。",
                        )
    ...
    async def uninstall_plugin(
        ...
        async with self._pm_lock:
            ...
            if not plugin:
                raise Exception("插件不存在。")
            if plugin.reserved:
                raise Exception("该插件是 NovaBot 保留插件，无法卸载。")
    ...
    async def _unbind_plugin(self, plugin_name: str, plugin_module_path: str) -> None:
        ...
        for func_tool in llm_tools.func_list:
            ...
            if (
                ...
                and not mp.endswith(("novabot.builtin_stars", "data.plugins"))
            ):
    ...
    async def update_plugin(
        ...
        if not plugin:
            raise Exception("插件不存在。")
        if plugin.reserved:
            raise Exception("该插件是 NovaBot 保留插件，无法更新。")
