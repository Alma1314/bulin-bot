...
from novabot.core.platform import Platform
from novabot.core.platform.nova_message_event import NovaMessageEvent, MessageSesion
from novabot.core.platform_message_history_mgr import PlatformMessageHistoryManager
from novabot.core.provider.entities import LLMResponse, ProviderRequest, ProviderType
from novabot.core.provider.func_tool_manager import FunctionTool, FunctionToolManager
from novabot.core.provider.manager import ProviderManager
from novabot.core.provider.provider import (
    EmbeddingProvider,
    Provider,
    RerankProvider,
    STTProvider,
...
from novabot.core.utils.novabot_path import get_novabot_system_tmp_path
...
if TYPE_CHECKING:
    from novabot.core.cron.manager import CronJobManager
...
class Context:
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            **kwargs: Additional keyword arguments. The kwargs will not be passed to the LLM directly for now, but can include:
                ...
                agent_hooks: BaseAgentRunHooks[NovaAgentContext] - hooks to run during agent execution
                agent_context: NovaAgentContext - context to use for the agent
        ...
        from novabot.core.nova_agent_context import (
            AgentContextWrapper,
            NovaAgentContext,
        )
        ...
        agent_hooks = kwargs.get("agent_hooks") or BaseAgentRunHooks[NovaAgentContext]()
        ...
        if agent_context is None:
            agent_context = NovaAgentContext(
                context=self,
                event=event,
            )
        ...
        if request.func_tool and request.func_tool.get_tool("novabot_file_read_tool"):
            other_kwargs.setdefault(
                "tool_result_overflow_dir", get_novabot_system_tmp_path()
            )
            other_kwargs.setdefault(
                "read_tool", request.func_tool.get_tool("novabot_file_read_tool")
            )
    ...
    def get_config(self, umo: str | None = None) -> NovaBotConfig:
        """获取 NovaBot 的配置。
        ...
        if not umo:
            # 使用默认配置
            return self._config
        return self.novabot_config_mgr.get_conf(umo)
    ...
    """
    以下的方法已经不推荐使用。请从 NovaBot 文档查看更好的注册方式。
    """
    ...
    def get_platform(self, platform_type: PlatformAdapterType | str) -> Platform | None:
        ...
        Note:
            该方法已经过时，请使用 get_platform_inst 方法。(>= NovaBot v4.0.0)
        """
        for platform in self.platform_manager.platform_insts:
    ...
    def register_llm_tool(
        ...
        Note:
            异步处理函数会接收到额外的关键词参数：event: NovaMessageEvent, context: Context。
            该方法已弃用，请使用新的注册方式。
        """
