...
from novabot.core import novabot_config, logger
from novabot.core.agent.runners.coze.coze_agent_runner import CozeAgentRunner
from novabot.core.agent.runners.dashscope.dashscope_agent_runner import (
    DashscopeAgentRunner,
)
from novabot.core.agent.runners.deerflow.constants import (
...
from novabot.core.agent.runners.dify.dify_agent_runner import DifyAgentRunner
from novabot.core.nova_agent_hooks import MAIN_AGENT_HOOKS
from novabot.core.message.components import Image, Record
...
class ThirdPartyAgentSubStage(Stage):
    async def initialize(self, ctx: PipelineContext) -> None:
        ...
        settings = ctx.novabot_config["provider_settings"]
    ...
    async def process(
        self, event: NovaMessageEvent, provider_wake_prefix: str
    ) -> AsyncGenerator[None, None]:
        ...
        self.prov_cfg: dict = next(
            (p for p in novabot_config["provider"] if p["id"] == self.prov_id),
            {},
        )
        if not self.prov_id:
        ...
        if self.runner_type == "dify":
            runner = DifyAgentRunner[NovaAgentContext]()
        elif self.runner_type == "coze":
            runner = CozeAgentRunner[NovaAgentContext]()
        elif self.runner_type == "dashscope":
            runner = DashscopeAgentRunner[NovaAgentContext]()
        elif self.runner_type == DEERFLOW_PROVIDER_TYPE:
            runner = DeerFlowAgentRunner[NovaAgentContext]()
        else:
            raise ValueError(
