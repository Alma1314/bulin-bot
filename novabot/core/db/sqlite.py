...
class SQLiteDatabase(BaseDatabase):
    ...
    async def get_platform_stats(self, offset_sec: int = 86400) -> list[PlatformStat]:
        ...
        async with self.get_db() as session:
            ...
            # Use subquery to get the latest record per platform_id (SQL-standard compliant)
            subq = (
                select(
                    PlatformStat.platform_id,
                    func.max(PlatformStat.timestamp).label("max_ts"),
                )
            ...
            result = await session.execute(
                select(PlatformStat).join(
                    subq,
                    (PlatformStat.platform_id == subq.c.platform_id)
                    & (PlatformStat.timestamp == subq.c.max_ts),
                ).order_by(PlatformStat.timestamp.desc()),
            )
