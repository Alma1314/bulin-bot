...
from novabot.core.nova_agent_context import AgentContextWrapper, NovaAgentContext
from novabot.core.nova_agent_hooks import MAIN_AGENT_HOOKS
from novabot.core.nova_agent_run_util import AgentRunner
from novabot.core.nova_agent_tool_exec import FunctionToolExecutor
from novabot.core.nova_main_agent_resources import (
...
from novabot.core.message.components import File, Image, Record, Reply, Video
...
from novabot.core.provider.register import llm_tools
from novabot.core.skills.skill_manager import (
    SkillInfo,
    SkillManager,
    build_skills_prompt,
)
from novabot.core.star.context import Context
from novabot.core.star.star import star_registry
from novabot.core.star.star_handler import star_map
from novabot.core.tools.computer_tools import (
...
from novabot.core.tools.knowledge_base_tools import (
...
from novabot.core.utils.novabot_path import (
    get_novabot_system_tmp_path,
    get_novabot_workspaces_path,
)
from novabot.core.utils.file_extract import extract_file_moonshotai
from novabot.core.utils.llm_metadata import LLM_METADATAS
from novabot.core.utils.media_utils import (
    IMAGE_COMPRESS_DEFAULT_MAX_SIZE,
    IMAGE_COMPRESS_DEFAULT_QUALITY,
    compress_image,
)
from novabot.core.utils.quoted_message.settings import (
...
def _set_llm_error_message(event: NovaMessageEvent, message: str) -> None:
    event.set_extra(LLM_ERROR_MESSAGE_EXTRA_KEY, message)


def _select_provider(
    event: NovaMessageEvent, plugin_context: Context
) -> Provider | None:
...
def _get_workspace_path_for_umo(umo: str) -> Path:
    ...
    return Path(get_novabot_workspaces_path()) / normalized_umo
        ...
        if skills:
            ...
            if runtime == "none":
                req.system_prompt += (
                    ...
                    "If you need to use these capabilities, ask the user to enable Computer Use in the NovaBot WebUI -> Config."
                )
...
def _plugin_tool_fix(event: NovaMessageEvent, req: ProviderRequest) -> None:
...
async def _handle_webchat(
    event: NovaMessageEvent, req: ProviderRequest, prov: Provider
) -> None:
    from novabot.core import db_helper
...
def _apply_sandbox_tools(
    ...
    if booter == "shipyard_neo":
        ...
        req.system_prompt += (
            ...
            "Preferred sequence:\n"
            "1) Use `novabot_create_skill_payload` to store canonical payload content and get `payload_ref`.\n"
            "2) Use `novabot_create_skill_candidate` with `skill_key` + `source_execution_ids` (and optional `payload_ref`) to create a candidate.\n"
            "3) Use `novabot_promote_skill_candidate` to release: `stage=canary` for trial; `stage=stable` for production.\n"
        ...
        from novabot.core.computer.computer_client import session_booter
    ...
    if booter == "cua":
        req.system_prompt += (
            "\n[CUA Desktop Control]\n"
            "Use `novabot_execute_shell` with `background=true` to launch GUI apps. "
            'Use Firefox for browser tasks, for example `firefox "https://example.com"`. '
            "After each visible step, call `novabot_cua_screenshot` with "
            ...
            "`novabot_cua_mouse_click` for coordinates and `novabot_cua_keyboard_type` "
...
def _get_compress_provider(
    config: MainAgentBuildConfig,
    plugin_context: Context,
    event: NovaMessageEvent | None = None,
) -> Provider | None:
    ...
    *,
    ...
    reset_coro = agent_runner.reset(
        ...
        tool_result_overflow_dir=(
            get_novabot_system_tmp_path()
            if req.func_tool and req.func_tool.get_tool("novabot_file_read_tool")
            else None
        ),
        read_tool=(
            req.func_tool.get_tool("novabot_file_read_tool") if req.func_tool else None
        ),
    )
