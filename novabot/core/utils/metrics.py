...
class Metric:
    ...
    def _is_disabled() -> bool:
        """检查是否禁用指标上传（配置或环境变量）"""
        if os.environ.get("NOVABOT_DISABLE_METRICS", "0") == "1":
            return True
        try:
            from novabot.core import novabot_config

            return novabot_config.get("disable_metrics", False)
    ...
    def get_installation_id():
        ...
        config_dir = os.path.join(os.path.expanduser("~"), ".novabot")
    ...
    async def upload(**kwargs) -> None:
        """上传相关非敏感的指标以更好地了解 NovaBot 的使用情况。上传的指标不会包含任何有关消息文本、用户信息等敏感信息。
