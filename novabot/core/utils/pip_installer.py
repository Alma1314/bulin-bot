...
from novabot.core.utils.novabot_path import get_novabot_site_packages_path
from novabot.core.utils.core_constraints import CoreConstraintsProvider
from novabot.core.utils.desktop_core_lock import get_desktop_core_lock_modules
    ...
    try:
        ...
        try:
            ...
            raise ImportError(
                ...
                f"NOVABOT_DESKTOP_CLIENT={os.environ.get('NOVABOT_DESKTOP_CLIENT')})"
            ) from exc
...
def _classify_pip_failure(output_lines: list[str]) -> DependencyConflictError | None:
    ...
    if is_core_conflict:
        message = (
            f"检测到核心依赖版本保护冲突。{detail}插件要求的依赖版本与 NovaBot 核心不兼容，"
...
class PipInstaller:
    def __init__(
        ...
        core_dist_name: str | None = "NovaBot",
    ) -> None:
    ...
    async def install(
        ...
        if is_packaged_desktop_runtime():
            target_site_packages = get_novabot_site_packages_path()
