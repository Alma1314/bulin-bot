...
from novabot.core.cron.events import CronMessageEvent
...
from novabot.core.provider.entites import ProviderRequest
...
from novabot.core.utils.history_saver import persist_agent_history
from novabot.core.utils.image_ref_utils import is_supported_image_ref
from novabot.core.utils.string_utils import normalize_and_dedupe_strings


class FunctionToolExecutor(BaseFunctionToolExecutor[NovaAgentContext]):
    @classmethod
    ...
    async def _collect_image_urls_from_message(
        cls, run_context: ContextWrapper[NovaAgentContext]
    ) -> list[str]:
        urls: list[str] = []
    ...
    async def _collect_handoff_image_urls(
        ...
        extensionless_local_roots = (get_novabot_temp_path(),)
        sanitized = [
            item
    ...
    async def execute(cls, tool, run_context, **tool_args):
        ...
        Args:
            event (NovaMessageEvent): 事件对象, 当 origin 为 local 时必须提供。
            **kwargs: 函数调用的参数。
    ...
    async def _wake_main_agent_for_background_result(
        ...
        *,
        ...
        from novabot.core.nova_main_agent import (
        ...
        if context:
            context_dump = req._print_friendly_context(contexts=context)
            req.system_prompt += (
