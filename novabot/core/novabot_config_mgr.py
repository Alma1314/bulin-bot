...
from novabot.core import NovaBotConfig, logger
from novabot.core.config.novabot_config import NOVABOT_CONFIG_PATH
...
from novabot.core.utils.novabot_path import get_novabot_config_path
...
DEFAULT_CONFIG_CONF_INFO = ConfInfo(
    id="default",
    name="default",
    path=NOVABOT_CONFIG_PATH,
)


class NovaBotConfigManager:
    """A class to manage the system configuration of NovaBot, aka ACM"""

    def __init__(
        self,
        default_config: NovaBotConfig,
        ucr: UmopConfigRouter,
        sp: SharedPreferences,
    ) -> None:
        self.sp = sp
        self.ucr = ucr
        self.confs: dict[str, NovaBotConfig] = {}
        """uuid / "default" -> NovaBotConfig"""
    ...
    def _load_all_configs(self) -> None:
        ...
        for uuid_, meta in abconf_data.items():
            filename = meta["path"]
            conf_path = os.path.join(get_novabot_config_path(), filename)
            if os.path.exists(conf_path):
                conf = NovaBotConfig(config_path=conf_path)
    ...
    def get_conf(self, umo: str | MessageSession | None) -> NovaBotConfig:
    ...
    @property
    def default_conf(self) -> NovaBotConfig:
        """获取默认配置文件"""
        return self.confs["default"]
    ...
    def create_conf(
        ...
        conf_path = os.path.join(get_novabot_config_path(), conf_file_name)
        conf = NovaBotConfig(config_path=conf_path, default_config=config)
        conf.save_config()
