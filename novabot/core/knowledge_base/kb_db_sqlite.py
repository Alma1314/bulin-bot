...
from novabot.core.db import _configure_sqlite_connection
...
from novabot.core.utils.novabot_path import get_novabot_knowledge_base_path

if TYPE_CHECKING:
    from novabot.core.db.vec_db.faiss_impl import FaissVecDB
...
class KBSQLiteDatabase:
    def __init__(self, db_path: str | None = None) -> None:
        ...
        Args:
            db_path: 数据库文件路径, 默认位于 NovaBot 数据目录下的 knowledge_base/kb.db

        """
        if db_path is None:
            db_path = str(Path(get_novabot_knowledge_base_path()) / "kb.db")
    ...
    async def delete_document_by_id(self, doc_id: str, vec_db: "FaissVecDB") -> None:
        ...
        async with self.get_db() as session, session.begin():
            ...
            # 删除关联的多媒体记录
            media_stmt = delete(KBMedia).where(col(KBMedia.doc_id) == doc_id)
