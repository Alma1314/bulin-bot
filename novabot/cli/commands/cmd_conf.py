...
def _load_config() -> dict[str, Any]:
    ...
    root = get_novabot_root()
    if not check_novabot_root(root):
        raise click.ClickException(
            f"{root} is not a valid NovaBot root directory. Use 'novabot init' to initialize",
        )
...
def _save_config(config: dict[str, Any]) -> None:
    """Save config file"""
    config_path = get_novabot_root() / "data" / "cmd_config.json"
