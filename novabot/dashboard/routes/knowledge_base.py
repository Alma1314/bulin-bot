...
from novabot.core.provider.provider import EmbeddingProvider, RerankProvider
...
class KnowledgeBaseRoute(Route):
    ...
    async def _background_upload_task(
        ...
        try:
            # 初始化任务状态
            self.progress.init_task(task_id, status="processing")
            ...
            for file_idx, file_info in enumerate(files_to_upload):
                try:
                    ...
                    # 创建进度回调函数
                    progress_callback = self.progress.make_callback(
                        task_id,
                        file_index=file_idx,
                        file_name=file_info["file_name"],
                    )
            ...
            self.progress.set_result(task_id, "completed", result=result)
    ...
    async def create_kb(self):
        ...
        try:
            ...
            try:
                vec = await prv.get_embedding("novabot")
            ...
            if rerank_provider_id:
                ...
                try:
                    res = await rerank_prv.rerank(
                        query="novabot",
                        documents=["novabot knowledge base"],
                    )
                    if not res:
    ...
    async def upload_document(self):
        ...
        try:
            ...
            # 初始化任务状态
            self.progress.init_task(task_id, status="pending")
    ...
    async def _background_upload_from_url_task(
        ...
        try:
            ...
            # 创建进度回调函数
            progress_callback = self.progress.make_callback(task_id, file_index=0, file_name=f"URL: {url}")
