...
from novabot.core import DEMO_MODE, logger, pip_installer
from novabot.core.config.default import VERSION
from novabot.core.core_lifecycle import NovaBotCoreLifecycle
from novabot.core.db.migration.helper import check_migration_needed_v4, do_migration_v4
...
class UpdateRoute(Route):
    def __init__(
        ...
        novabot_updator: NovaBotUpdator,
        ...
        self.novabot_updator = novabot_updator
    ...
    async def do_migration(self):
        ...
        try:
            ...
            await do_migration_v4(
                ...
                pim,
                self.core_lifecycle.novabot_config,
            )
    ...
    async def check_update(self):
        ...
        try:
            ...
            ret = await self.novabot_updator.check_update(None, None, False)
            return Response(
                status="success",
    ...
    async def get_releases(self):
        try:
            ret = await self.novabot_updator.get_releases()
    ...
    async def update_project(self):
        ...
        try:
            ...
            await self.novabot_updator.update(
                latest=latest,
                version=version,
                proxy=proxy,
                progress_callback=self._make_progress_callback(
                ...
                self.update_progress[progress_id].update(
                    {
                        "status": "success",
                        "stage": "done",
                        "message": "更新成功，NovaBot 将在 2 秒内全量重启以应用新的代码。",
                        "overall_percent": 100,
                    },
                )
                ret = (
                    Response()
                    .ok(None, "更新成功，NovaBot 将在 2 秒内全量重启以应用新的代码。")
                    .__dict__
                )
            ...
            self.update_progress[progress_id].update(
                {
                    "status": "success",
                    "stage": "done",
                    "message": "更新成功，NovaBot 将在下次启动时应用新的代码。",
                    "overall_percent": 100,
                },
            )
            ret = (
                Response()
                .ok(None, "更新成功，NovaBot 将在下次启动时应用新的代码。")
                .__dict__
            )
            return ret, 200, CLEAR_SITE_DATA_HEADERS
        except Exception as e:
