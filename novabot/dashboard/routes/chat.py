...
from novabot.core.agent.message import get_checkpoint_id, is_checkpoint_message
...
from novabot.core.platform.sources.webchat.message_parts_helper import (
    build_webchat_message_parts,
    create_attachment_part_from_existing_file,
    strip_message_parts_path_fields,
    webchat_message_parts_have_content,
)
from novabot.core.platform.sources.webchat.webchat_queue_mgr import webchat_queue_mgr
from novabot.core.utils.active_event_registry import active_event_registry
from novabot.core.utils.novabot_path import get_novabot_data_path
from novabot.core.utils.datetime_utils import to_utc_isoformat

from .route import Response, Route, RouteContext

# SSE heartbeat message to keep the connection alive during long-running operations

# --- 三个聊天路由 (chat / live_chat / open_api) 共享的定义和工具 ---

# 附件类型 → 消息中标签前缀 映射
ATTACHMENT_PREFIX_MAP: dict[str, str] = {
    "image": "[IMAGE]",
    "record": "[RECORD]",
    "file": "[FILE]",
    "video": "[VIDEO]",
...
# should_save 判定中排除的 chain_type（非终结性中间消息）
_NON_TERMINAL_CHAIN_TYPES: tuple[str, ...] = ("tool_call", "tool_call_result")


def attachment_prefix_for(msg_type: str) -> str:
    """返回附件消息类型对应的标签前缀。"""
    return ATTACHMENT_PREFIX_MAP.get(msg_type, "")
...
def should_save_message(
    ...
    """判断当前消息是否应该持久化保存。"""
    if msg_type == "end":
        return bool(has_accumulated or has_refs or has_stats)
    if (streaming and msg_type == "complete") or not streaming:
        return chain_type not in _NON_TERMINAL_CHAIN_TYPES
    return False


# --- Progress Tracker (backup / knowledge_base 共享) ---
...
class ProgressTracker:
    ...
    def init_task(self, task_id: str, *, status: str = "pending", **extra) -> None:
        """初始化任务条目。"""
        self._tasks[task_id] = {"status": status, "result": None, "error": None, **extra}
    ...
    def make_callback(self, task_id: str, **context):
        """创建进度回调函数。context 中的额外字段会被合并到进度更新中。"""

        async def _cb(stage: str, current: int, total: int, message: str = "") -> None:
    ...
    def get_task(self, task_id: str) -> dict | None:
        ...
        if info["status"] == "processing" and task_id in self._progress:
            info["progress"] = dict(self._progress[task_id])
        return info
...
class ChatRoute(Route):
    def __init__(
        ...
        db: BaseDatabase,
        core_lifecycle: NovaBotCoreLifecycle,
    ) -> None:
        ...
        self.attachments_dir = os.path.join(get_novabot_data_path(), "attachments")
        self.legacy_img_dir = os.path.join(get_novabot_data_path(), "webchat", "imgs")
    ...
    async def get_file(self):
        ...
        try:
            ...
            if os.path.commonpath([real_file_path, real_imgs_dir]) != real_imgs_dir:
    ...
    async def get_attachment(self):
        ...
        try:
            ...
            real_file_path = os.path.realpath(file_path)
            real_attachments_dir = os.path.realpath(self.attachments_dir)

            if os.path.commonpath([real_file_path, real_attachments_dir]) != real_attachments_dir:
                return Response().error("Invalid file path").__dict__
    ...
    def _extract_web_search_refs(
        ...
        """委托给模块级函数。"""
        return extract_web_search_refs(accumulated_text, accumulated_parts)
            ...
            try:
                ...
                async with track_conversation(self.running_convs, webchat_conv_id):
                    while True:
                        ...
                        elif msg_type in ATTACHMENT_PREFIX_MAP:
                            prefix = ATTACHMENT_PREFIX_MAP[msg_type]
                            filename = result_text.replace(prefix, "")
                        ...
                        should_save = should_save_message(
                            msg_type=msg_type,
                            streaming=streaming,
                            chain_type=chain_type,
                            has_accumulated=message_accumulator.has_content(),
                            has_refs=bool(refs),
                            has_stats=bool(agent_stats),
                        )
