...
from novabot.api.event import NovaMessageEvent, filter
from novabot.api.message_components import Image, Plain
from novabot.api.provider import ProviderRequest
from novabot.core import logger
from novabot.core.utils.session_waiter import (
    FILTERS,
    USER_SESSIONS,
    SessionController,
    SessionWaiter,
...
def _iter_message_components(event: NovaMessageEvent):
...
class Main(star.Star):
    def __init__(self, context: star.Context) -> None:
        ...
        try:
            self.group_chat_context = GroupChatContext(
                self.context.novabot_config_mgr,
                self.context,
            )
    ...
    async def handle_session_control_agent(self, event: NovaMessageEvent) -> None:
        """会话控制代理"""
    ...
    def group_context_enabled(self, event: NovaMessageEvent):
    ...
    async def decorate_llm_req(
        self, event: NovaMessageEvent, req: ProviderRequest
    ) -> None:
    ...
    async def after_message_sent(self, event: NovaMessageEvent) -> None:
        """消息发送后处理"""
        if self.group_chat_context and self.group_context_enabled(event):
            try:
