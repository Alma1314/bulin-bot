...
from novabot.api.event import NovaMessageEvent
from novabot.api.message_components import At, Image, Plain
from novabot.api.platform import MessageType
from novabot.api.provider import Provider, ProviderRequest
...
class GroupChatContext:
    def __init__(self, acm: NovaBotConfigManager, context: star.Context) -> None:
        self.acm = acm
    ...
    def cfg(self, event: NovaMessageEvent):
    ...
    async def need_active_reply(self, event: NovaMessageEvent) -> bool:
        cfg = self.cfg(event)
    ...
    async def remove_session(self, event: NovaMessageEvent) -> int:
    ...
    async def handle_message(self, event: NovaMessageEvent) -> None:
        if event.get_message_type() != MessageType.GROUP_MESSAGE:
            return
    ...
    async def on_req_llm(self, event: NovaMessageEvent, req: ProviderRequest) -> None:
        umo = event.unified_msg_origin
    ...
    async def _format_message(self, event: NovaMessageEvent, cfg: dict) -> str:
