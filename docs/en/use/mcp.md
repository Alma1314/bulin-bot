...
If you're deploying NovaBot with Docker, you can also execute the following command for quick installation:

```bash
docker exec nova-bot python -m pip install uv
```

If you're deploying NovaBot from source, please install it within the created virtual environment.

For `npm`, you need to install `node`.

If you're deploying NovaBot from source or using one-click installation, please refer to [Download Node.js](https://nodejs.org/en/download) to download to your local machine.

If you're using Docker to deploy NovaBot, you need to install `node` in the container (future NovaBot Docker images will include `node` by default). Please execute the following commands:

```bash
sudo docker exec -it nova-bot /bin/bash
...
After installing `node`, you need to restart `NovaBot` to apply the new environment variables.

## Installing MCP Servers

If you're deploying NovaBot with Docker, please install MCP servers in the data directory.
