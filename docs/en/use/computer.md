...
- `local`: executes on the host machine where NovaBot is running. Use this when the Agent needs local files, command-line tools, or local dependencies.
...
`local` mode mounts Computer Use tools into the host environment where NovaBot runs. The Agent can call the host Shell, host Python, and host filesystem tools.

This means the Agent's boundary is close to the NovaBot process itself. What it can access depends on the system permissions, runtime user, working directory, and operating-system restrictions of the NovaBot process.

### Workspace

In `local` mode, NovaBot prepares a workspace for each session:
...
> [!NOTE]
> The local Python tool executes code through NovaBot's current Python environment. When Python code reads or writes files, use explicit absolute paths or prepare files through filesystem tools in the workspace first.
...
- `Python`: executes Python code in NovaBot's current Python environment.
...
- `Require NovaBot admin permission`
...
- NovaBot temporary directories
- `.nova-bot` under the system temporary directory

If `Require NovaBot admin permission` is disabled, regular users behave much closer to admins for Computer Use tools. Do not disable it unless you understand the risk.
...
`sandbox` mode runs execution actions inside an isolated environment instead of directly on the NovaBot host.

Inside the sandbox, the Agent can still use Shell, Python, and filesystem tools. If the selected sandbox profile supports the `browser` capability, NovaBot also mounts browser automation tools.
...
For sandbox deployment, profiles, TTL, persistence, and browser capabilities, see [Agent Sandbox Environment](/en/use/nova-bot-agent-sandbox).

> [!NOTE]
> Even in `sandbox` mode, `Require NovaBot admin permission` still affects access to Shell, Python, browser, upload/download, and related tools. The exact behavior depends on your configuration.
...
In `sandbox` mode, NovaBot attempts to sync local Skills into the sandbox so the Agent can execute them there.
