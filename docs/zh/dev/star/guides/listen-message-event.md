...
NovaBot 接收消息平台下发的消息，并将其封装为 `BulinMessageEvent` 对象，传递给插件进行处理。

![message-event](https://files.bulinbot.app/docs/zh/dev/star/guides/message-event.svg)

### 消息事件

`BulinMessageEvent` 是 NovaBot 的消息事件对象，其中存储了消息发送者、消息内容等信息。

### 消息对象

`NovaBotMessage` 是 NovaBot 的消息对象，其中存储了消息平台下发的消息具体内容，`BulinMessageEvent` 对象中包含一个 `message_obj` 属性用于获取该消息对象。
