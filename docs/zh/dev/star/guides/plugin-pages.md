...
NovaBot 支持插件通过 `pages/` 目录暴露 Dashboard 页面。`pages/` 下的每个一级子目录都是一个独立 Page：
...
NovaBot 会扫描 `pages/<page_name>/index.html`；没有 `index.html` 的目录会被忽略。

如果只是让用户填写几个配置项，优先使用 [`_conf_schema.json`](./plugin-config.md)。插件 Pages 更适合复杂表单、Dashboard、日志、文件上传下载、SSE 和自定义交互流程。

一旦注册了 Pages，用户可以在：NovaBot WebUI 插件页中的插件卡片中，点击插件卡片进入插件详细页面，在插件详细页面中可以看到并进入注册的 Pages。
...
这里不需要手动引入 bridge SDK。NovaBot 会在返回的 HTML 里自动插入 `/api/plugin/page/bridge-sdk.js`。
...
插件 Page 中可直接使用 `window.NovaBotPluginPage`：
...
插件 Page 复用插件 i18n 资源文件。给 `.nova-bot-plugin/i18n/<locale>.json` 增加 `pages.<page_name>` 即可：
...
如果你的内联脚本需要同步访问 `window.NovaBotPluginPage`，请把脚本放到外部 module 文件中，或在自己的脚本前显式引入：
...
NovaBot 切换亮色/暗色模式时，会自动将主题状态同步给插件 Page。bridge SDK 会在 `<html>` 元素上维护 `data-theme` 属性：
...
NovaBot 在服务端返回 HTML 时，会自动在 `<html>` 标签上注入 `data-theme` 属性，不会出现初始闪烁。
...
NovaBot 会重写相对资源路径，并自动补上短期 `asset_token`。你只需要正常写相对路径，不要自己拼接 `/api/plugin/page/content/...`。
