...
![image](https://files.novabot.app/docs/source/images/plugin/image.png)
...
`NovaMessageEvent` 是 NovaBot 的消息事件对象。你可以通过 `NovaMessageEvent` 来获取消息发送者、消息内容等信息。
...
async def on_message(self, event: NovaMessageEvent):
...
![插件 logo 示例](https://files.novabot.app/docs/source/images/plugin/plugin_logo.png)
...
```python
@filter.command("echo")
async def echo(self, event: NovaMessageEvent, message: str):
...
@calc.command("help")
def calc_help(self, event: NovaMessageEvent):
    # /math calc help
...
def help(self, event: NovaMessageEvent):
    yield event.plain_result("这是一个计算器插件，拥有 add, sub 指令。")
```
...
async def on_agent_begin(self, event: NovaMessageEvent, run_context: ContextWrapper[NovaAgentContext]): # 请注意有三个参数
    print("Agent 开始运行")
```
...
async def on_agent_done(self, event: NovaMessageEvent, run_context: ContextWrapper[NovaAgentContext], resp: LLMResponse): # 请注意有四个参数
    print(resp)
```
...
![发送 QQ 表情](https://files.novabot.app/docs/source/images/plugin/image-5.png)
...
![image](https://files.novabot.app/docs/source/images/plugin/QQ_1738149538737.png)
...
from nova-bot.api.event import NovaMessageEvent
...
class HelloWorldTool(FunctionTool):
    ...
    async def run(
        self,
        event: NovaMessageEvent, # 必须包含此 event 参数在前面，用于获取上下文
