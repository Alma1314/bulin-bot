...
飞书（Lark）支持两种创建方式：在 NovaBot 中扫码一键创建，或在飞书开发者后台手动创建企业自建应用。

### 方式一：扫码一键创建

需要版本 >4.25.0。

进入 NovaBot 管理面板，点击左边栏 `机器人`，然后点击 `+ 创建机器人`，选择 `lark(飞书)`。

在 `选择创建方式` 中选择 `扫码一键创建`，按需选择国内版或海外版，然后使用手机飞书扫描页面中的二维码并确认。创建成功后，NovaBot 会自动写入该应用的 `app_id`、`app_secret` 和域名配置。
...
>
> App ID 获取方式：回到 NovaBot 的 `机器人` 页，找到刚刚创建的飞书机器人，点击 `编辑`，弹出的对话框中可以看到 App ID。
>
> ```text
...
![创建企业自建应用](https://files.novabot.app/docs/source/images/lark/image.png)

添加应用能力——机器人。

![添加应用能力](https://files.novabot.app/docs/source/images/lark/image-1.png)

点击凭证与基础信息，获取 app_id 和 app_secret。

![获取 app_id 和 app_secret](https://files.novabot.app/docs/source/images/lark/image-4.png)
...
如果您选择了 `webhook` 方式，选择了之后，前往飞书的开发者后台，点击事件与回调，点击加密策略，填写 Encrypt Key。这不是必须的，NovaBot 十分注重你的数据安全，所以请务必填写。填写后复制 `Encrypt Key` 和 `Verification Token` 到 NovaBot 配置的 `encrypt_key` 和 `verification_token` 处。
...
![设置事件与回调](https://files.novabot.app/docs/source/images/lark/image-6.png)
...
![](https://files.novabot.app/docs/source/images/lark/webhook.png)
...
![添加事件](https://files.novabot.app/docs/source/images/lark/image-7.png)

点击开通以下权限。

![开通权限](https://files.novabot.app/docs/source/images/lark/image-8.png)
...
![最终开通的权限](https://files.novabot.app/docs/source/images/lark/image-11.png)

## 创建版本

创建版本。

![创建版本](https://files.novabot.app/docs/source/images/lark/image-2.png)
...
搜索刚刚创建的机器人的名字。比如教程创建了 `NovaBot` 机器人：

![添加机器人](https://files.novabot.app/docs/source/images/lark/image-9.png)

## 🎉 大功告成

在群内发送一个 `/help` 指令，机器人将做出响应。

![成功](https://files.novabot.app/docs/source/images/lark/image-13.png)
