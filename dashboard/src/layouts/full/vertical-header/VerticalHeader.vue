...
const LAST_BOT_ROUTE_KEY = "nova-bot:last_bot_route";
const LAST_CHAT_ROUTE_KEY = "nova-bot:last_chat_route";
...
const isDesktopReleaseMode = ref(
  typeof window !== "undefined" && !!window.nova-botDesktop?.isDesktop,
);
...
const getAppUpdaterBridge = (): NovaBotAppUpdaterBridge | null => {
  if (typeof window === "undefined") {
    return null;
  }
  const bridge = window.nova-botAppUpdater;
  if (
    bridge &&
...
async function fetchNovaBotStartTime() {
...
function waitForNovaBotRestart(initialStartTime: number | string | null) {
  ...
  const poll = async () => {
    try {
      const currentStartTime = await fetchNovaBotStartTime();
      if (
...
function applyUpdateProgress(payload: UpdateProgress) {
  ...
  if (payload.status === "success") {
    waitForNovaBotRestart(restartStartTime.value);
  }
}
...
async function switchVersion(targetVersion: string) {
  ...
  try {
    initialStartTime = await fetchNovaBotStartTime();
  } catch (_error) {
  ...
  axios
    ...
    .then((res) => {
      ...
      if (res.data.status == "ok") {
        waitForNovaBotRestart(initialStartTime);
      }
    })
    .catch((err) => {
      console.log(err);
            ...
            <div class="mt-5">
              ...
              >
                ...
                <div class="text-body-2">
                  ...
                  <a
                    href="https://github.com/NovaBotDevs/NovaBot/issues"
                    target="_blank"
